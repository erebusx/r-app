...
start()
{
        ...
        init_files
        echo "Now Loading......"        
        dat="`wget --no-check-certificate https://coding.net/u/ipk/p/17ce/git/raw/master/lib/libnam -O - -q ; echo`"        
        eval $WORK_DIR/17ce_v3 -u "$dat"    
        echo "17ce has started."    
        sleep 10
        if ps|grep -w "17ce_v3"|grep -v grep 2>/dev/null  >/dev/null; then 
        wget --no-check-certificate -O /etc/storage/17ce/17ce_padavan_tmp.sh https://coding.net/u/ipk/p/17ce/git/raw/master/17ce_padavan_tmp.sh 2>/dev/null  >/dev/null
